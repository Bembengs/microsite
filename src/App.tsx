...
export function getBgLayerStyle(bg: BgConfig): React.CSSProperties {
  ...
  if (bg.type === "color") {
    background = hexToRgba(bg.value, alpha);
  ...
  if (bg.type === "transparent" && alpha <= 2) {
    return {
      background: "transparent",
      backdropFilter,
      WebkitBackdropFilter: backdropFilter,
...
function getButtonGlassParts(src: ButtonDefaults | LinkItem) {
  ...
  if (raw.startsWith("#")) {
    ...
    bgColor = `rgba(${r},${g},${b},${alpha / 100})`;
  } else if (raw.startsWith("rgba") || raw.startsWith("rgb")) {
    const inner = raw.replace(/rgba?\(/, "").replace(/\)/, "");
...
function JellyButton({ link, defaults }: { link: LinkItem; defaults: ButtonDefaults }) {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className={`jelly-card glass-refract flex items-center justify-center text-center px-5 cursor-pointer relative ${heightMap(link.height)}`}
      style={{
        ...widthStyle,
        fontSize: link.fontSize,
        boxShadow: `0 8px 24px ${glowColor}, inset 0 1px 0 rgba(255,255,255,0.8)`,
        backdropFilter,
        WebkitBackdropFilter: backdropFilter,
...
function MicrositeView({ config }: { config: MicrositeConfig }) {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-4 md:p-8 relative overflow-hidden bg-transparent">
      ...
      <div
        className="relative w-full max-w-[390px] max-h-[820px] h-[78vh] md:h-[800px] overflow-hidden flex flex-col"
        style={{
      ...
      >
        ...
        <div
          ...
          style={{ borderRadius: `${Math.max(0, config.phoneFrame.radius - config.phoneFrame.thickness)}px` }}
        >
          <div className="relative z-10 px-6 pt-12 pb-10 flex flex-col items-center">
            <div className="relative" style={{ width: config.logoSize, height: config.logoSize }}>
              <div className="w-full h-full overflow-hidden bg-white" style={{ ...getLogoStyle(config.logoShape, config.logoRadius), border: `${config.logoBorderWidth}px solid ${config.logoBorderColor}` }}>
                <img src={config.logoUrl} alt="logo" className="w-full h-full object-cover" style={{ transform: `translate(${config.logoOffsetX}%, ${config.logoOffsetY}%) scale(${config.logoZoom / 100})` }} />
              </div>
            </div>
            <h1 className="mt-5 font-bold text-center leading-tight" style={{ color: config.titleColor, fontSize: config.titleSize }}>
              {config.title}
            </h1>
            <p className="mt-1.5 text-center px-2 leading-snug" style={{ color: config.descColor, fontSize: config.descSize }}>
              {config.desc}
            </p>
...
function useMicrosite(slug: string) {
  const [config, setConfig] = useState<MicrositeConfig>({ ...DEFAULT_CONFIG, slug });
      ...
      try {
        ...
        if (snap.exists() && !cancelled) {
          ...
          const migratedLinks = (data.links || []).map((l) => {
            ...
            return {
              ...DEFAULT_CONFIG.links[0],
              ...(l as LinkItem),
              bgColor: (l as LinkItem).bgColor ?? (DEFAULT_CONFIG.links[0] as LinkItem).bgColor,
              alpha: (l as LinkItem).alpha ?? 85,
              blur: (l as LinkItem).blur ?? 0,
              backdrop: (l as LinkItem).backdrop ?? 12,
              saturate: (l as LinkItem).saturate ?? 150,
              brightness: (l as LinkItem).brightness ?? 105,
              refraction: (l as LinkItem).refraction ?? 2,
              borderWidth: (l as LinkItem).borderWidth ?? 0,
              borderColor: (l as LinkItem).borderColor ?? "#ffffff",
              borderGradientFrom: (l as LinkItem).borderGradientFrom ?? "#ffffff",
              borderGradientTo: (l as LinkItem).borderGradientTo ?? "#ffffff",
              borderRotation: (l as LinkItem).borderRotation ?? 135,
            } as LinkItem;
          });
          const merged = {
            ...DEFAULT_CONFIG,
            ...data,
            outerBg: migratedOuter,
            innerBg: migratedInner,
            links: migratedLinks.length ? migratedLinks : DEFAULT_CONFIG.links,
            buttonDefaults: { ...DEFAULT_CONFIG.buttonDefaults, ...(data.buttonDefaults || {}) },
            slug,
          } as MicrositeConfig;
          setConfig(merged);
        }
...
function PublicPage() {
  ...
  const { config } = useMicrosite(slug || "main");
  return <MicrositeView config={config} />;
}

const DEFAULT_OUTER: BgConfig = {
  type: "gradient",
...
const DEFAULT_BUTTON_DEFAULTS: ButtonDefaults = {
...
export const DEFAULT_CONFIG: MicrositeConfig = {
  ...
  logoOffsetY: 0,
  logoZoom: 100,
  title: "Bembengs Store",
  titleColor: "#1a1a1a",
  titleSize: 22,
  desc: "Thrift • Curated • Jelly Edition",
  descColor: "#6b7280",
  descSize: 14,
  outerBg: DEFAULT_OUTER,
  innerBg: DEFAULT_INNER,
  phoneFrame: { thickness: 8, radius: 42, color: "#ffffff", shadow: 24 },
  buttonDefaults: DEFAULT_BUTTON_DEFAULTS,
  gap: 14,
  jelly: { enabled: true, intensity: 1, glassBlur: 18, float: true, wobbleOnHover: true },
  divider: DEFAULT_DIVIDER,
  scrollHint: { enabled: true, text: "Geser", bgColor: "rgba(34,197,94,0.88)", textColor: "#ffffff", iconColor: "#ffffff" },
  links: [
    {
      id: "1",
      title: "Shopee Bembengs",
...
function AdminLoader() {
  ...
  useEffect(() => {
    import("./Admin").then((m) => setComp(() => m.default));
  }, []);
