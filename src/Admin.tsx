    ...
    <div className="p-3 rounded-2xl bg-white border shadow-sm space-y-3">
      ...
      <div className="space-y-2 pt-2 border-t">
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px]">
          ...
          <label className="flex items-center gap-1.5 mt-5"><input type="checkbox" checked={!!config.logoAnimationOnHover} onChange={(e) => onChange({ ...config, logoAnimationOnHover: e.target.checked })} /> Hanya saat hover</label>
        </div>
        <p className="text-[10px] opacity-50">Tips: float & jelly paling cocok untuk brand thrift. spin untuk promo.</p>
      </div>
      <div className="grid grid-cols-2 gap-2 text-[11px]">
...
function BgGlassEditor({ label, bg, onChange }: { label: string; bg: BgConfig; onChange: (b: BgConfig) => void }) {
  ...
  const handlePaste = () => {
    ...
    const isVid = isVideoUrl(pasting.trim());
    onChange({ ...bg, type: isVid ? "video" : "image", value: pasting.trim() });
  };
      ...
      {(bg.type === "image" || bg.type === "video") && (
        <div className="space-y-2">
          <div className="flex gap-2">
            <input value={pasting} onChange={(e) => setPasting(e.target.value)} placeholder={bg.type === "video" ? "https://.../video.mp4" : "https://.../gambar.jpg atau video.mp4"} className="flex-1 p-2.5 rounded-xl border bg-white text-sm font-mono" />
            <button onClick={handlePaste} className="px-3 py-2 rounded-xl bg-black text-white text-xs font-bold">Pakai</button>
          </div>
          {isVideoUrl(pasting) && <p className="text-[10px] text-green-600">Terdeteksi video — akan pakai tag video</p>}
        </div>
      )}
      {bg.type === "transparent" && <p className="text-[11px] opacity-60">Mode kaca murni — hanya backdrop blur + alpha putih tipis</p>}
      <button type="button" onClick={() => onChange({ ...bg, hue: 0, alpha: 100, opacity: 100, blur: 0, backdrop: 0, saturate: 100, brightness: 100, refraction: 0 })} className="text-[11px] px-2.5 py-1 rounded-full bg-black/[0.06] hover:bg-black/[0.10] border font-bold w-fit">↺ Reset ke asli (6 slider)</button>
...
function ButtonDefaultsEditor({ defaults, onChange }: { defaults: ButtonDefaults; onChange: (d: ButtonDefaults) => void }) {
  return (
    <div className="p-3 rounded-2xl bg-white border shadow-sm space-y-3">
      <p className="font-semibold text-sm">🔘 Button Default — jelly</p>
      <div className="grid grid-cols-2 gap-2 text-[11px]">
        ...
        <label className="space-y-1">blur {defaults.blur}px<input type="range" min={0} max={30} value={defaults.blur} onChange={(e) => onChange({ ...defaults, blur: Number(e.target.value) })} className="w-full" /></label>
        <label className="space-y-1">saturate {defaults.saturate}%<input type="range" min={0} max={300} value={defaults.saturate} onChange={(e) => onChange({ ...defaults, saturate: Number(e.target.value) })} className="w-full" /></label>
        <label className="space-y-1">brightness {defaults.brightness}%<input type="range" min={0} max={200} value={defaults.brightness} onChange={(e) => onChange({ ...defaults, brightness: Number(e.target.value) })} className="w-full" /></label>
        <label className="space-y-1">refraction {defaults.refraction}<input type="range" min={0} max={20} value={defaults.refraction} onChange={(e) => onChange({ ...defaults, refraction: Number(e.target.value) })} className="w-full" /></label>
        ...
        <label className="space-y-1">shape<input type="text" value={defaults.shapeType} onChange={(e) => onChange({ ...defaults, shapeType: e.target.value as ButtonDefaults["shapeType"] })} className="w-full p-1 border rounded" /></label>
        <label className="space-y-1">torn {defaults.tornAmount}<input type="range" min={0} max={100} value={defaults.tornAmount} onChange={(e) => onChange({ ...defaults, tornAmount: Number(e.target.value) })} className="w-full" /></label>
        <label className="flex items-center gap-1"><input type="checkbox" checked={defaults.bold} onChange={(e) => onChange({ ...defaults, bold: e.target.checked })} /> bold</label>
        <label className="flex items-center gap-1"><input type="checkbox" checked={defaults.italic} onChange={(e) => onChange({ ...defaults, italic: e.target.checked })} /> italic</label>
      </div>
    </div>
  );
}
...
function SortableLink({ block, onUpdate, onRemove }: { block: LinkItem; onUpdate: (b: LinkItem) => void; onRemove: () => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="p-3 rounded-2xl bg-white border shadow-sm space-y-2">
      <div className="flex items-center justify-between"><div className="flex items-center gap-2"><span {...attributes} {...listeners} className="cursor-grab px-2">↕</span><span className="font-semibold text-sm">{block.title || "Link"}</span></div><button onClick={onRemove} className="text-[11px] px-2 py-1 rounded-full bg-red-50 border text-red-600">hapus</button></div>
      ...
      <div className="grid grid-cols-2 gap-2 text-[11px]">
        <label className="space-y-1">width {block.width}%<input type="range" min={30} max={100} value={block.width} onChange={(e) => onUpdate({ ...block, width: Number(e.target.value) })} className="w-full" /></label>
        <label className="space-y-1">fontSize {block.fontSize}px<input type="range" min={10} max={24} value={block.fontSize} onChange={(e) => onUpdate({ ...block, fontSize: Number(e.target.value) })} className="w-full" /></label>
      </div>
    </div>
  );
}
...
function SortableDivider({ block, onUpdate, onRemove }: { block: DividerBlock; onUpdate: (b: DividerBlock) => void; onRemove: () => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="p-3 rounded-2xl bg-white border shadow-sm space-y-2">
      <div className="flex items-center justify-between"><div className="flex items-center gap-2"><span {...attributes} {...listeners} className="cursor-grab px-2">↕</span><span className="font-semibold text-sm">Pembatas</span></div><button onClick={onRemove} className="text-[11px] px-2 py-1 rounded-full bg-red-50 border text-red-600">hapus</button></div>
...
export default function Admin() {
    ...
    (async () => {
      try {
        ...
        if (ds.exists()) {
          ...
          // FIX ISSUE 2: safe merge agar tidak blank kalau data corupt / outerBg null
          ...
          // FIX blank: kalau doc sudah dihapus, fallback ke default (bukan kosong)
  ...
  @keyframes logoGlow{0%,100%{box-shadow:0 0 12px rgba(255,255,255,0.6)}50%{box-shadow:0 0 20px rgba(255,255,255,0.9)}}
  ...
  const handlePublish = async () => {
    setSaving(true);
    const targetUrl = `/${slug === "main" ? "" : slug}`;
    const win = window.open("about:blank", "_blank");
    try {
      const ref = doc(db, "microsites", slug);
      await setDoc(ref, { ...config, slug, updatedAt: Date.now() });
      const col = collection(db, "microsites");
      const snap = await getDocs(col);
      setAllSlugs(snap.docs.map((d) => d.id));
      navigate(`/admin/${slug}`);
    ...
    <div className="min-h-screen w-full flex bg-[#f6f5f2]">
      ...
      <div className="w-[420px] max-h-screen overflow-y-auto p-4 space-y-4 border-r bg-[#fdf2f8]">
        <div className="p-3 rounded-2xl bg-white border shadow-sm space-y-2">
          ...
          <div className="flex gap-2"><input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="slug" className="flex-1 p-2 rounded-lg border text-sm" /></div>
          <div className="flex gap-1 flex-wrap">{allSlugs.map((s) => <button key={s} onClick={() => { setSlug(s); navigate(`/admin/${s}`); }} className={`px-2.5 py-1 rounded-full border text-[11px] ${slug === s ? "bg-black text-white" : "bg-white"}`}>{s}</button>)}</div>
          <p className="text-[10px] opacity-60">Jika slug dihapus manual di Firestore, klik tetap aman — akan fallback ke template default (tidak blank).</p>
        </div>
